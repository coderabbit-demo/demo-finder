...
USE_CASES: list[dict] = [
    ...
    dict(slug="mcp-client", name="MCP Client", category="integrations",
         definition="Connect CodeRabbit to external tools and data sources through Model Context Protocol. CodeRabbit acts as the client and uses server tools to enrich reviews, validate suggestions, and answer PR chat with relevant external context.",
         doc_url=f"{DOCS}/connections/mcp-servers",
         detection_heuristics={"needs_linked_issue": True,
                               "keywords": ["issue", "linear", "notion", "confluence", "mcp"]},
    ...
    dict(slug="multi-repo", name="Multi-repo / Org-wide Context", category="context",
         definition="Multi-Repo Analysis lets CodeRabbit use linked repositories as review context so it can identify cross-repository impacts such as shared contracts, clients, SDKs, and dependent services.",
         doc_url=f"{DOCS}/knowledge-base/multi-repo-analysis",
    ...
    dict(slug="atlas-change-stack", name="Change Stack", category="review",
         definition="Change Stack reorganizes a pull request into a guided, layer-by-layer review so related changes can be read in a meaningful order with code context, findings, and review actions in one place.",
         doc_url=f"{DOCS}/change-stack",
         detection_heuristics={"min_files": 6, "needs_multi_service": True,
                               "keywords": ["stack", "layer", "architecture", "service", "flow"]},
         required_config={},
         demo_script_notes="Use a multi-file PR; open Review Change Stack and move through its semantic layers."),
    dict(slug="security-blast-radius", name="Security Blast Radius", category="security",
         definition="Security Blast Radius maps how pull request changes connect to upstream dependencies, downstream consumers, tests, and security-relevant paths. Every component and relationship links to supporting code evidence.",
         doc_url=f"{DOCS}/security/blast-radius",
         detection_heuristics={"min_files": 3, "needs_multi_service": True,
                               "keywords": ["shared", "dependency", "consumer", "route", "auth", "security"]},
         required_config={},
         demo_script_notes="Use a cross-cutting security change with a shared component and several consumers; open Security Blast Radius in Change Stack."),
    dict(slug="security-architecture-review", name="Security Architecture Review", category="security",
         definition="Security Architecture Review assesses implications that span changed components, trust boundaries, security controls, data flows, and downstream dependencies.",
         doc_url=f"{DOCS}/security/architecture-review",
         detection_heuristics={"min_files": 3, "needs_multi_service": True,
                               "keywords": ["auth", "permission", "boundary", "gateway", "token", "policy"]},
         required_config={},
         demo_script_notes="Use a PR that changes an entry point, trust boundary, and downstream authorization behavior."),
    dict(slug="security-attack-surface", name="Security Attack Surface", category="security",
         definition="The attack surface map organizes security-relevant code into subsystems and tracks verification coverage, findings, and drift as pull requests merge.",
         doc_url=f"{DOCS}/security/attack-surface",
         detection_heuristics={"min_files": 4,
                               "keywords": ["entrypoint", "api", "route", "auth", "network", "security"]},
         required_config={},
         demo_script_notes="Open the repository attack-surface map and trace an exposed subsystem from entry point to finding coverage."),
    dict(slug="security-deep-scan", name="AI Deep Scan", category="security",
         definition="AI Deep Scan analyzes committed source code and infrastructure configuration for exploitable vulnerabilities and exposed secrets beyond the current pull request diff.",
         doc_url=f"{DOCS}/security",
         detection_heuristics={"keywords": ["vulnerability", "secret", "injection", "terraform", "kubernetes", "scan"]},
         required_config={},
         demo_script_notes="Repository-level demo: run AI Deep Scan, then inspect reachability, exploitability, evidence, and remediation from Agent Findings."),
]
